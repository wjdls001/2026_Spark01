  ...
  return (
    <div className="flex min-h-dvh flex-col bg-white">
      <div className="flex items-center gap-3 px-5 pt-5 pb-3">
        ...
        <h1 className="text-lg font-bold text-spark-dark">루틴 불러오기</h1>
      </div>

      <div className="flex-1 px-5 py-2">
        {routines.length === 0 ? (
          <p className="py-10 text-center text-sm text-spark-gray">불러올 루틴이 없어요.</p>
        ) : (
          <div className="flex flex-col gap-3">
            {routines.map(r => (
              ...
              >
                <div>
                  <div className="text-sm font-bold text-spark-dark">{r.name}</div>
                  <div className="mt-0.5 text-xs text-spark-text-secondary">{r.sets}세트 × {r.reps}회</div>
                </div>
